...
amazon_purchases_file = "~/Downloads/Retail.OrderHistory.1.csv"

# This is the csv file with kind purchases should be (when downloaded it's in Digital-Ordering.1/Digital Items.csv
kindle_purchases_file = "~/Downloads/Digital-Ordering.1/Digital Items.csv"
    ...
    for index, row in df.iterrows():
        if row["Title"] not in exclude_list:
            ...
            if count < intermediate_count + 10: 
                print (row["Title"], row["OurPrice"], row["OurPriceCurrencyCode"], row["FulfilledDate"])
            if count > 1:
                output_json = output_json + " , "
            output_json = output_json + digitalPurchaseExtract(row["Title"], row["OurPrice"], row["OurPriceCurrencyCode"], row["FulfilledDate"]).toJson()
